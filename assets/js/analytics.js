...
function renderMembersChart() {
  try {
    ...
    const chartContainer = document.getElementById('membersChart');
    ...
    const activePercentCalc = totalMembers > 0 ? ((activeMembers / totalMembers) * 100).toFixed(2) : 0;
    const expiresSoonPercentCalc = totalMembers > 0 ? ((expiresSoonMembers / totalMembers) * 100).toFixed(2) : 0;
    const expiredPercentCalc = totalMembers > 0 ? ((expiredMembers / totalMembers) * 100).toFixed(2) : 0;
